...
const MediaQuery = () => {
  const mediaQueryContext =
    ...
    if (typeof window !== "undefined" && window.matchMedia) {
      React.useEffect(() => {
        const creteMatchMedia = (bp: BreakpointRange, min?: number, max?: number): [MediaQueryList, (this: MediaQueryList, eve?: MediaQueryListEvent) => void] => {
          const query = `screen${(min) ? ` and (min-width: ${min}px)` : ""}${(max) ? ` and (max-width: ${max}px)` : ""}`;
          const mql = window.matchMedia(query);
          const callback = function(this: MediaQueryList, eve?: MediaQueryListEvent) {
        ...
        const mqls: {mql: MediaQueryList, callback: (this: MediaQueryList, eve: MediaQueryListEvent) => void}[] = [];

        let oldWidth: number | undefined;
        for(const [bp, width] of Object.entries(breakpoints)) {
          const [mql, callback] = creteMatchMedia(bp as BreakpointRange, oldWidth, width);
